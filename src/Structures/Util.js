...
module.exports = class Util {
  ...
  keygen(length) {
    let result = "";
    const characters =
      "ABCDEFGHIJKLMNOPQRSTUVQXYZabcdefghijklmnpqrstuvqxyz1234567890";
      ...
      for (const commandFile of commands) {
        ...
        if (!command.data)
          return this.client.logger.log(
            `Command ${commandFile} doesn't export slashcommand data.`
          );
        this.client.slashCommands.set(command.data.name, command);
        commandData.push(command.data);
        this.client.logger.log(
          `${this.capitalise(command.data.name)} Slashcommand Loaded`
        );
      }
    });
  ...
  async loadCommands() {
    return glob(`${this.directory}Commands/**/*.js`).then((commands) => {
      for (const commandFile of commands) {
        ...
        this.client.logger.log(
          `${this.capitalise(command.name)} Command Loaded`
        );
