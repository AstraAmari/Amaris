...
module.exports = class extends Event {
   ...
   */
  async run(message) {
    const { guildId, channelId, channel, author, content, member, guild } =
      message;
    if (
      author.bot ||
    ...
    if (
      ...
      if (
        guildSettings?.mentionMemberEnabled &&
        message.mentions.members.size > memberLimit
      ) {
        return message
          .delete()
          .catch(() => {
            channel.send(
              `${author}, you're mentioning too many members!\nMessage could not be deleted.`
            );
          })
          .then(() => {
            channel.send(
        ...
        guildSettings?.mentionRoleEnabled &&
        message.mentions.roles.size > roleLimit
      ) {
        return message
          .delete()
          .catch(() => {
            return channel.send(
              `${author}, you're mentioning too many roles!\nMessage could not be deleted.`
            );
          })
          .then(() => {
            return channel.send(
    ...
    if (guildSettings?.spamEnabled) {
      ...
      if (spamMap.has(author.id)) {
        ...
        const difference =
          message.createdTimestamp - userData.lastMessage.createdTimestamp;
          ...
          if (userData.msgCount === limit) {
            try {
              ...
              return message.channel
                ...
                .catch(() => {
                  return channel.send(
                    `${author}, you're sending messages too quickly! Please slow down.\n${messages.size} messages could not be deleted...`
                  );
                })
                .then(() => {
                  return channel.send(
    ...
    if (guildSettings?.zalgoEnabled) {
      ...
      if (regex.test(encodeURIComponent(content))) {
        return message
          ...
          .catch(() => {
            channel.send(
              `${author}, zalgo text is not allowed!\nMessage could not be deleted.`
            );
          })
          .then(() => {
            channel.send(
      ...
      if (content.length >= 6) {
        ...
        if (capsPercent > percentage) {
          return message
            ...
            .catch(() => {
              channel.send(
                `${author}, you're using too many CAPITALS!\nMessage could not be deleted.`
              );
            })
            .then(() => {
              channel.send(
    ...
    if (guildSettings?.emojiEnabled) {
      ...
      if (emojiList && emojiList?.length > limit) {
        return message
          ...
          .catch(() => {
            channel.send(
              `${author}, you're using too many emojis!\nMessage could not be deleted.`
            );
          })
          .then(() => {
            channel.send(
