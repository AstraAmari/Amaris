  ...
  async run(member) {
    ...
    if (guildSettings?.autobanEnabled) {
      ...
      if (age < minimum) {
        await user
          .send(
            `You were banned from **${member.guild.name}** because your account needs to be at least ${minimum} days old!`
          )
          .catch(() => null);
        await member
          .ban({ reason: "AutoBan: Account too young!" })
          .catch(() => null);
      }
    }
    if (guildSettings?.zalgoNicknamesEnabled) {
      ...
      if (regex.test(encodeURIComponent(member?.nickname))) {
        member
          .setNickname(
            decodeURIComponent(
              encodeURIComponent(member.nickname.replace(/[^\x00-\x7F]/g, ""))
