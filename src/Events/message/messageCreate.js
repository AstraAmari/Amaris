  ...
  async run(message) {
    ...
    if (message.content.match(mentionRegex)) {
      return message.channel.send(
        `My prefix for ${message.guild.name} is \`${guildPrefix}\`.`
      );
    }
    ...
    if (this.client.blackList.includes(message.author.id)) {
      this.client.logger.warn(
        `blocked user : ${message.author.id} | ${message.author.tag} tried to run a command`
      );
    ...
    if (command) {
      ...
      client.logger.cmd(
        `${client.utils.capitalise(command.name)} Command Executed\n\t${
          msg.author.id
