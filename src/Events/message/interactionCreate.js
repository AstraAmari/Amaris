  ...
  async run(interaction) {
    ...
    if (this.client.blackList.includes(interaction.user.id)) {
      this.client.logger.warn(
        `blocked user : ${interaction.user.id} | ${interaction.user.tag} tried to run a command`
      );

      return await interaction.reply({
        content: `you are blocked from using ${this.client.user.username} \nif you think this is a mistake please contact my developer via the support server https://discord.gg/uRYUSqmPrn`,
      });
    }
    ...
    if (command) {
      if (
        command.ownerOnly &&
        !this.client.utils.checkOwner(interaction.user.id)
      ) {
      ...
      if (
        (command.guildOnly && !interaction.guild) ||
        (command.guildOnly && !interaction.inCachedGuild())
      ) {
        return interaction.reply(
      ...
      client.logger.cmd(
        `${client.utils.capitalise(
          command.data.name
        )} Slashcommand Executed\n\t${interaction.user.id}\t${
          interaction.user.tag
        }\n\t${interaction.channel ? interaction.channel.id : ""}\t${
          interaction.channel ? `#${interaction.channel.name}` : ""
        }${
          interaction.guild
            ? `\n\t${interaction.guild.id}\t${interaction.guild.name}`
            : ""
        }`,
        { shard: interaction.guild?.shard.id ?? 0 }
      );
      ...
      if (maintanence === true) {
        return interaction.reply({
          content: `maintanence mode is active, you cannot use this bot\'s commands during this time.\n For more information join our support server: https://discord.gg/uRYUSqmPrn`,
        });
      }
      ...
      try {
        ...
        interaction.reply(["An unexpected error has occured!"].join("\n"));
