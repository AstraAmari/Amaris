...
module.exports = class extends Command {
  ...
  async run(message, args) {
    ...
    if (userEconomy.lastRobDate) {
      ...
      if (nextRobData > now)
        return message.channel.send(
          `You can rob again in ${Ms(nextRobData - now)}`
        );
    }
    ...
    if (otherUser) {
      ...
      if (!otherUserEconomy)
        return message.channel.send(
          `Please add a user id or mention of a user you would like to rob`
        );
    }
    ...
    if (caught) {
      ...
      return message.channel.send(
        `You got caught and lose ${loss} Paradise Coin`
      );
    }
    ...
    return message.channel.send(
      `You robbed ${amountRob} Paradise Coin from ${otherUser}`
    );
  }
};
