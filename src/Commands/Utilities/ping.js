...
module.exports = class extends Command {
  ...
  async run(message) {
    ...
    const e = new EmbedBuilder()
      ...
      .addFields({
        name: "Bot Latency:",
        value: [`${latency.toLocaleString()}ms`].join("\n"),
      })

      .addFields({
        name: "API Latency:",
        value: [`${Math.round(this.client.ws.ping).toLocaleString()}ms`].join(
          "\n"
        ),
      })
      ...
      .addFields({
        name: `Shard Latency`,
        value: [`${Math.round(msg.guild.shard.ping).toLocaleString()}ms`].join(
          "\n"
        ),
      })
      ...
      .addFields({
        name: `Server TPS`,
        value: [`${i.toLocaleString()}TPS`].join("\n"),
      })
