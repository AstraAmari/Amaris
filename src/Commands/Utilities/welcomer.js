  ...
  async run(message, args) {
    ...
    if (!args.length) {
        ...
        .addFields(
          {
            ...
            value: `\`joinchannel <channel>\` - sets the join channel\n\`resetjoinchannel\` - resets the join channel\n*Current Channel:* ${
              guildSettings.welcomerJoinChannel
                ? `<#${guildSettings.welcomerJoinChannel}>`
                : "None"
            }`,
          },
          {
            name: "Message",
            value: `\`joinmessage <message>\` - sets the join message\n*Current Message:*\n${guildSettings.welcomerJoinMessage}`,
          },
          {
            name: "Placeholders",
            value: [
      ...
      const leaveSettingsEmbed = new EmbedBuilder()
        .setColor("Red")
        .setAuthor({
          name: guild.name + " | Leave Settings",
          iconURL: guild.iconURL(),
        })
        .addFields(
          {
            ...
            value: `\`leavechannel <channel>\` - sets the leave channel\n\`resetleavechannel\` - resets the leave channel\n*Current Channel:* ${
              guildSettings.welcomerLeaveChannel
                ? `<#${guildSettings.welcomerLeaveChannel}>`
                : "None"
            }`,
          },
          {
            name: "Message",
            value: `\`leavemessage <message>\` - sets the leave message\n*Current Message:*\n${guildSettings.welcomerLeaveMessage}`,
          },
          {
            name: "Placeholders",
            value: [
        ...
        .addFields(
          {
            name: "Commands",
            value: [
              `\`addrole <@role>\` - Adds a role to the list`,
              `\`removerole <id>\` - Removes a role from the list`,
            ].join("\n"),
          },
          {
            name: `Roles (${guildSettings.welcomerJoinRoles.length} of 30)`,
            value: `${
              guildSettings.welcomerJoinRoles.length
                ? guildSettings.welcomerJoinRoles
                    .map((id) => `<@&${id}>`)
                    .join(" ")
                : "None"
      ...
      return message.reply({
        embeds: [joinSettingsEmbed, leaveSettingsEmbed, joinRolesEmbed],
      });
    } else if (args[0].toLowerCase() === "joinmessage") {
      const msg = args.slice(1).join(" ");
      if (!msg.length || msg.length > 600)
        return message.reply(
          "Invalid or no message provided! If your message is over 600 charectors it will not be set"
        );
      ...
      if (!guildSettings.welcomerLeaveChannel)
        return message.reply("There is no channel set!");
      guildSettings.welcomerLeaveChannel = undefined;
      guildSettings.save(guildSettings);
      message.reply(`The channel was reset!`);
      ...
      if (guildSettings.welcomerJoinRoles.length >= 30)
        return message.reply("The list can not have more than 30 roles");
      if (guildSettings.welcomerJoinRoles.includes(role.id))
        return message.reply("The role is already on the list");
      guildSettings.welcomerJoinRoles.push(role.id);
      settings.save(guildSettings);
      message.reply("The role was added!");
    } else if (args[0].toLowerCase() === "removerole") {
      const role = args[1];
      if (!role || !guildSettings.welcomerJoinRoles.includes(role))
        return message.reply("The role is not on the list");
      guildSettings.welcomerJoinRoles = guildSettings.welcomerJoinRoles.filter(
        (id) => id != role
      );
      settings.save(guildSettings);
