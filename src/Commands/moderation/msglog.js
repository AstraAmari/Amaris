...
module.exports = class extends LogCommand {
  constructor(...args) {
    ...
    this.client.on("messageUpdate", async (oldMessage, newMessage) => {
      ...
      await this.log(
        oldMessage.guild.id,
        `${oldMessage.author.tag} | (${oldMessage.author.id}) edited a message \nbefore: ${oldMessage.content}\n\nafter: ${newMessage.content}`
      ).catch(() => null);
    });
    ...
    this.client.on("messageDelete", async (oldMessage) => {
      ...
      await this.log(
        oldMessage.guild.id,
        `${oldMessage.author.tag} | (${oldMessage.author.id}) deleted a message \n${oldMessage.content}`
      ).catch(() => null);
    });
  ...
  async run(message, [cmd, channel]) {
    ...
    return message.channel.send(
      `Message log is ${cache.has(message.guild.id) ? "enabled" : "disabled"}`
    );
  }
