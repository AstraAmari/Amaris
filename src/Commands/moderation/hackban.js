  ...
  async run(message, args) {
    ...
    if (member && !member.bannable) {
      ...
      await this.client.modLog(
        message.guild.id,
        `${message.author} banned ${user} ${reason}`
      );

      await message.channel.send(
        `You have banned ${member.user.tag} | ${member.user.id} ${reason}`
      );
    } else {
      message.guild.members.ban(user.id).catch(() => null);
      await this.client.modLog(
        message.guild.id,
        `${message.author} hack banned ${user} ${reason}`
      );

      await message.channel.send(
        `You have hackbanned ${user.tag} | ${user.id} ${reason}`
      );
    }
  }
};
